...
// Optional auth: if Authorization header is missing, request is treated as anonymous.
// If header exists but token is invalid, reject with 401 (prevents "half-authenticated" behavior).
export const optionalAuthMiddleware = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    authedReq.user = { id: internalUser.id, role: internalUser.role as AuthUser['role'] }
